		...
		<div className="flex flex-col gap-4 lg:w-full">
			<Container>
				<InputWrapper title="Data Pribadi">
					...
					<InputProfile title="Jenis Kelamin">{datapribadi?.jenis_kelamin || "-"}</InputProfile>
					<InputProfile title="No Identitas">{datapribadi?.no_identitas || "-"}</InputProfile>
					<InputProfile title="NPWP">{datapribadi?.npwp || "-"}</InputProfile>
					<InputProfile title="Agama">{datapribadi?.agama || "-"}</InputProfile>
					<InputProfile title="Alamat KTP">{datapribadi?.alamat_ktp || "-"}</InputProfile>
					<InputProfile title="Alamat Domisili">{datapribadi?.alamat_domisili || "-"}</InputProfile>
					<InputProfile title="No Telepon">{datapribadi?.no_telepon || "-"}</InputProfile>
					<InputProfile title="Tempat Lahir">{datapribadi?.tempat_lahir || "-"}</InputProfile>
					<InputProfile title="Tanggal Lahir">
						{datapribadi?.tgl_lahir ? new Date(datapribadi.tgl_lahir).toLocaleDateString() : "-"}
					</InputProfile>
					<InputProfile title="Perusahaan">{datapribadi?.perusahaan?.nama || "-"}</InputProfile>
				</InputWrapper>
			</Container>
			...
			<Container>
				<InputWrapper title="Data Pegawai" onTab={"1"}>
					<InputProfile title="Jabatan">{datapegawai?.jabatan?.nama || "-"}</InputProfile>
					<InputProfile title="Departemen">{datapegawai?.departemen?.nama || "-"}</InputProfile>
					<InputProfile title="Divisi">{datapegawai?.divisi?.nama || "-"}</InputProfile>
					<InputProfile title="Unit">{datapegawai?.unit?.nama || "-"}</InputProfile>
					<InputProfile title="Tanggal Bergabung">
						{datapegawai?.tgl_bergabung ? new Date(datapegawai.tgl_bergabung).toLocaleDateString() : "-"}
					</InputProfile>
					<InputProfile title="Tanggal Resign">
						{datapegawai?.tgl_resign ? new Date(datapegawai.tgl_resign).toLocaleDateString() : "-"}
					</InputProfile>
				</InputWrapper>
			</Container>
			...
			<Container>
				<InputWrapper title="Data Keluarga" onTab={"2"}>
					<InputProfile title="Nama Ayah">{datakeluarga?.nama_ayah || "-"}</InputProfile>
					<InputProfile title="Nama Ibu">{datakeluarga?.nama_ibu || "-"}</InputProfile>
					<InputProfile title="Status Pernikahan">{datakeluarga?.status_pernikahan || "-"}</InputProfile>
					<InputProfile title="Nama Pasangan">{datakeluarga?.nama_pasangan || "-"}</InputProfile>
					<InputProfile title="Jumlah Anak">{datakeluarga?.anak || "-"}</InputProfile>
					...
					{formalEducation && formalEducation.length > 0 ? (
						formalEducation.map((item, index) => (
							<div key={index} className="flex flex-col gap-4">
								<InputProfile title="Asal Sekolah">{item.asal_sekolah || "-"}</InputProfile>
								<InputProfile title="Masa Waktu">{item.masa_waktu || "-"}</InputProfile>
				...
				<InputWrapper title="Data lainnya" onTab={"4"}>
					{datalainnyaNew && datalainnyaNew.length > 0 ? (
						datalainnyaNew.map((item, index) => (
							<div key={index} className="flex flex-col gap-4">
								<div className="flex items-center w-full gap-2">
									<p className="w-[40%] md:w-3/6 text-left capitalize text-sm">Data ke {index + 1}</p>
									<FileInput
										height={100}
										accept={{ "image/jpeg": [], "image/png": [] }}
										disabled={true}
			...
			<>
				<div className="flex flex-row justify-between items-center">
					...
					{data?.groups[0]?.name !== "Super Admin" && (
						<Link
							to={`/kepegawaian/data-pegawai/form/${data?.datapribadi?.no_identitas}`}
				...
				<div className="flex flex-col gap-4 justify-between w-full">{children}</div>
			</>
		);
	}
