...
const DetailPayrollGajiPage = () => {
	...
	const handlePrint = useReactToPrint({
		content: () => ref.current,
		documentTitle: `Slip Gaji ${detailPayroll?.employee_first_name} Periode ${moment(
			detailPayroll?.payroll_period,
			"YYYY-MM"
		).format("MMMM YYYY")}`, // Set the document title directly
	});

	// Jika ada Pk maka Fetching data lama
	const get = useCallback(async () => {
		if (decrypted_id(pk)) {
			const response = await axiosAPI.get(API_URL_payroll + decrypted_id(pk) + "/");
		...
		<div>
			...
			<div className="shadow-lg rounded-lg bg-white dark:bg-base-600 overflow-hidden">
				<div className=" flex items-start justify-center px-6 md:px-10">
					<Button className={"flex flex-row items-center gap-2"} size="xs" onClick={handlePrint}>
						<FaPrint /> Print
					</Button>
				</div>
				<div className="flex justify-center">{detailPayroll && <Slip ref={ref} data={detailPayroll} />}</div>
			</div>
		</div>
	);
};
