...
const MasterGajiPage = () => {
	const { getMasterGajiResult, addMasterGajiResult, deleteMasterGajiResult, getMasterGajiLoading } = useSelector(
		(state) => state.mastergaji
	);
	...
	const onEdit = (item) => {
		...
		navigate(`/payroll/master-gaji/form/${encrypted_id(item.id)}`);
	};
	...
	const doDelete = (item) => {
		deleteData({ dispatch, redux: masterGajiReducer }, item.id, API_URL_edeluser, "DELETE_AKUN");
	};

	const get = useCallback(
		async (param) => {
			await getData({ dispatch, redux: masterGajiReducer }, param, API_URL_salary, "GET_MASTERGAJI");
			setLoading(false);
		},
		[dispatch]
	);
	...
	const getDetail = async (id) => {
		...
		try {
			...
			alert(error.response.data.error || "Terjadi Kesalahan saat mengambil detail gaji pegawai");
		}
	...
	}, [addMasterGajiResult, deleteMasterGajiResult, search, limit, pageActive, get]);
		...
		<div>
							...
							{dataWithIndex.length > 0 ? (
								dataWithIndex.map((item) => (
									<Tables.Row key={item.id}>
										...
										<Tables.Data>{item?.employee_name || "Nama tidak tersedia"}</Tables.Data>
			...
			<Modal show={showModal} setShow={setShowModal} width="md" btnClose={true} persistent={false}>
				<div className="p-6 bg-white rounded-lg shadow-lg">
					<h2 className="text-2xl font-semibold mb-4 text-center">Detail Gaji</h2>
					<ul className="space-y-4">
						...
						{jwt?.level === "Super Admin" && (
							<li className="flex justify-between">
								...
								<span className="text-gray-800">{detailGaji?.nama_perusahaan}</span>
							</li>
						)}
						<li className="flex justify-between">
						...
						<li className="flex justify-between">
							...
							<span className="text-gray-800">{detailGaji?.jabatan_pegawai}</span>
						</li>
						<li className="flex justify-between">
							<span className="font-medium">Gaji:</span>
							<span className="text-gray-800">{formatRupiah(detailGaji?.amount)}</span>
						</li>
					</ul>
				</div>
			</Modal>
