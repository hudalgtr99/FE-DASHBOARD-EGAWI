...
const UnitSubForm = () => {
	const { addUnitLoading } = useSelector((state) => state.organ);
	const { pk } = useParams();
			...
			try {
				if (isEdit) {
					...
					if (data && !addUnitLoading) {
						navigate("/master-data/organization", { state: { activeTab: "2" } });
					}
				} else {
				...
				<div>
					<form onSubmit={formik.handleSubmit} className="space-y-6">
						<Select
							...
							name="departemen"
							value={
								departemenOptions.find((option) => option.value === formik.values.departemen) || null
							}
						...
						<Select
							...
							name="divisi"
							value={
								filteredDivisiOptions.find((option) => option.value === formik.values.divisi) || null
							}
						...
						<div className="mt-6 flex justify-end">
							<Button loading={addUnitLoading} type="submit">
