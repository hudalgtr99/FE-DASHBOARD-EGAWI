...
const DivisiSub = () => {
	...
	const onAdd = () => {
		navigate("/master-data/organization/divisi/form");
	};

	const onEdit = (item) => {
		navigate(`/master-data/organization/divisi/form/${item.slug}`, {
			state: {
				item,
			},
		});
	...
	const doDelete = (item) => {
		deleteData({ dispatch, redux: divisiReducers }, item.slug, API_URL_edeldivisi, "DELETE_DIVISI");
	};

	const get = useCallback(
		async (param) => {
			setLoading(true); // Set loading to true
			await getData({ dispatch, redux: divisiReducers }, param, API_URL_getdivisi, "GET_DIVISI");
							...
							{dataDevisi.length > 0 ? (
								dataDevisi.map((item) => (
									<Tables.Row key={item.pk}>
										...
										{!jwt.perusahaan && (
											<Tables.Data>{item?.departemen?.perusahaan?.nama || ""}</Tables.Data>
										)}
