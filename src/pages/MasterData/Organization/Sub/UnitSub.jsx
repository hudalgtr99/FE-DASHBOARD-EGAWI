...
import { API_URL_edelunit, API_URL_getunit, API_URL_getperusahaan } from "@/constants";
...
const UnitSub = () => {
	...
	const onAdd = () => {
		navigate("/master-data/organization/unit/form");
	};

	const onEdit = (item) => {
		navigate(`/master-data/organization/unit/form/${item.slug}`, {
			state: {
				item,
			},
		});
	...
	const doDelete = (item) => {
		deleteData({ dispatch, redux: unitReducers }, item.slug, API_URL_edelunit, "DELETE_UNIT");
	};

	const get = useCallback(
		async (param) => {
			setLoading(true); // Set loading to true before fetching
			await getData({ dispatch, redux: unitReducers }, param, API_URL_getunit, "GET_UNIT");
							...
							{dataWithIndex.length > 0 ? (
								dataWithIndex.map((item) => (
									<Tables.Row key={item.pk}>
										...
										<Tables.Data>
											{(item?.divisi?.departemen && item?.divisi?.departemen.nama) || "-"}
										</Tables.Data>
