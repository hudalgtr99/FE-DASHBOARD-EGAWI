...
const JabatanSub = () => {
	...
	const onAdd = () => {
		...
		navigate("/master-data/jabatan/form", {
			state: {
				item,
			},
		});
	...
	const onEdit = (item) => {
		...
		navigate(`/master-data/jabatan/form/${encrypted_id(item?.pk)}`, {
			state: {
				item,
			},
		});
	...
	const doDelete = (item) => {
		deleteData({ dispatch, redux: jabatanReducers }, item.pk, API_URL_edeljabatan, "DELETE_JABATAN");
	};

	const get = useCallback(
		async (param) => {
			getData({ dispatch, redux: jabatanReducers }, param, API_URL_getjabatan, "GET_JABATAN");
		},
		[dispatch]
	);
	...
	useEffect(() => {
		if (addJabatanResult || addPangkatResult || deleteJabatanResult || deletePangkatResult) {
			const param = search
				? {
	...
	}, [addJabatanResult, addPangkatResult, deleteJabatanResult, deletePangkatResult, search, limit, pageActive, get]);
