...
const LokasiAbsenPage = () => {
	...
	const onAdd = () => {
		...
		navigate("/master-data/lokasi-absen/form", {
			state: {
				item,
			},
		});
	...
	const onEdit = (item) => {
		sessionStorage.setItem("url", location.pathname);
		navigate(`/master-data/lokasi-absen/form/${item.slug}`, {
			state: {
				item,
			},
		});
	...
	}, [addperusahaanResult, deleteperusahaanResult, selectedPerusahaan, search, limit, pageActive, get]);
