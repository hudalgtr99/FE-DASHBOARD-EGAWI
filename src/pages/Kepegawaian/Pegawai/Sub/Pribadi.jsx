...
const Pribadi = ({ onTabChange }) => {
	const { pk } = useParams();
	const { addPegawaiResult, addPegawaiLoading } = useSelector((state) => state.kepegawaian);
	...
	const formik = useFormik({
		...
		validationSchema: Yup.object().shape({
			...
			username: Yup.string().required("Nama wajib diisi").max(255, "Nama harus kurang dari 255 karakter"),
		...
		onSubmit: async (values) => {
			try {
				const updatedValues = {
					...values,
					lokasi_absen_ids: JSON.stringify(values.lokasi_absen.map((option) => option.value)),
		...
		const fetchData = async () => {
			...
			if (storedData && options.length === 1) {
				formik.setFieldValue("perusahaan", {
					value: options.find((opt) => opt.pk === formik.values?.perusahaan?.pk).value,
				});
				const parsedData = JSON.parse(storedData);
		...
		const fetchData = async () => {
			...
			if (storedData) {
				...
				if (Object.keys(parsedData.datapribadi.groups).length === 0) {
					parsedData.datapribadi.groups = roles2.find((roles) => roles.name === "Pegawai");
					localStorage.setItem("editUserData", JSON.stringify(parsedData));
				}
			}
		};
		...
		<div>
			<Container>
				<div className="flex items-center gap-2 mb-4">
					...
					<h1>{sessionStorage.getItem("isEdit") === "true" ? "Edit Data Pribadi" : "Tambah Data Pribadi"}</h1>
				</div>
				<div>
					<form
						onSubmit={(e) => {
					...
					>
						<div className="sm:flex block sm:gap-4 max-[640px]:space-y-4">
							<Select
								...
								onChange={(option) => formik.setFieldValue("perusahaan", option ? option : "")}
						...
						<div className="sm:flex block sm:gap-4 max-[640px]:space-y-4">
							...
							<Select
								...
								onChange={(option) => formik.setFieldValue("lokasi_absen", option ? option : "")} // Mengambil value dari option yang dipilih
