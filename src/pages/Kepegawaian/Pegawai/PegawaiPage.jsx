...
const AkunPage = () => {
	...
	const get = useCallback(
		async (param) => {
			await getData({ dispatch, redux: userReducer }, param, API_URL_getdatapegawai, "GET_AKUN");
	...
	const onAdd = () => {
		...
		sessionStorage.removeItem("activeTab");
		navigate("/kepegawaian/data-pegawai/form");
	};
				...
				<div className="mb-4 flex flex-col sm:flex-row justify-center sm:justify-between items-center gap-4">
					...
					<div className="flex gap-2 items-center">
						<Tooltip tooltip="Import Pegawai">
							<Button onClick={() => navigate("/kepegawaian/data-pegawai/import-pegawai")} size="36">
							...
							{dataWithIndex.length > 0 ? (
								dataWithIndex.map((item) => (
									<Tables.Row key={item.datapribadi.user_id}>
										...
										{!jwt.perusahaan && (
											<Tables.Data>{item?.datapribadi?.perusahaan?.nama || "N/A"}</Tables.Data>
										)}
										...
										<Tables.Data>
											{item.datapribadi?.no_telepon || "No telepon tidak tersedia"}
										</Tables.Data>
										<Tables.Data>
