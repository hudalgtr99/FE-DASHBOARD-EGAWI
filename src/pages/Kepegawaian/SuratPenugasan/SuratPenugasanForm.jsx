...
const SuratPenugasanSlug = () => {
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			if (isEdit) {
				...
				if (data && !addTugasLoading) {
					navigate(
						sessionStorage.getItem("url") ? sessionStorage.getItem("url") : "/master-data/surat-penugasan"
					);
					sessionStorage.removeItem("url");
				}
			}
	...
	useEffect(() => {
		const fetchAllData = async () => {
			try {
				const [pegawaiResponse, jumlahSuratResponse, templatesResponse] = await Promise.all([
				...
				if (jumlahSurat >= 0) {
					const id_template = formik.values.template.toString().padStart(2, "0");
					const id_templateEdit = isEdit ? formik.values.template.id : {}.toString().padStart(2, "0");
		...
		<Container>
          ...
          <Tooltip placement="left" tooltip="Lihat tutorial">
			...
			<form onSubmit={formik.handleSubmit} className="space-y-6">
				<TextField label="Nomor Surat" name="noSurat" value={formik.values.no_surat} disabled />
				<TextField
					required
					label="Nama Surat"
					name="nama"
				...
				<Select
					...
					value={
						pegawaiOptions.find(
							(option) =>
								option.value === formik.values.pemohon.id || option.value === formik.values.pemohon
						) || null
					}
					onChange={(option) => {
				...
				<Select
					...
					value={templatesOptions.find(
						(option) =>
							option.value === formik.values.template.id || option.value === formik.values.template
					)}
					onChange={(option) => {
				...
				<CKEditor
					...
					pemohon={pegawaiOptions.find((option) => option.value === formik.values.pemohon)}
					penerima={pegawaiOptions.filter((option) =>
						formik.values.penerima.some((penerimaItem) => penerimaItem.value === option.value)
					)}
