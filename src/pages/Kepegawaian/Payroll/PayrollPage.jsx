...
const JobdeskPegawaiPage = () => {
	const { getJobdeskPegawaiResult, addJobdeskPegawaiResult, deleteJobdeskPegawaiResult, getJobdeskPegawaiLoading } =
	...
	const onEdit = (item) => {
		...
		navigate(`/kepegawaian/jobdesk-pegawai/form/${encrypted_id(item.id)}`);
	};
	...
	const doDelete = (item) => {
		deleteData({ dispatch, redux: jobdeskPegawaiReducer }, item.id, API_URL_edeluser, "DELETE_AKUN");
	};
	...
	const getDetail = async (id) => {
		...
		try {
			...
			alert(error.response.data.error || "Terjadi Kesalahan saat mengambil detail jobdesk pegawai");
		}
	...
	}, [addJobdeskPegawaiResult, deleteJobdeskPegawaiResult, search, limit, pageActive, get]);
	...
	const onAdd = () => {
		navigate(`/kepegawaian/jobdesk-pegawai/form`);
	};
	...
	return (
		<div>
					...
					<Tables>
						<Tables.Head>
							<tr>
								...
								{jwt?.level === "Super Admin" && <Tables.Header>Nama perusahaan</Tables.Header>}
								<Tables.Header>Id Pegawai</Tables.Header>
								...
								dataWithIndex.map((item) => (
									<Tables.Row key={item.id}>
										...
										{jwt?.level === "Super Admin" && (
											<Tables.Data>{item?.nama_perusahaan || "N/A"}</Tables.Data>
										)}
										<Tables.Data>{item?.id_pegawai || "belum ada"}</Tables.Data>
										<Tables.Data>{item?.first_name || "Nama tidak tersedia"}</Tables.Data>
			...
			<Modal show={showModal} setShow={setShowModal} width="md" btnClose={true} persistent={false}>
				<div className="p-6 bg-white rounded-lg shadow-lg">
					<h2 className="text-2xl font-semibold mb-4 text-center">Detail Jobdesk</h2>
					<ul className="space-y-4">
						...
						{jwt?.level === "Super Admin" && (
							<li className="flex justify-between">
								...
								<span className="text-gray-800">{detailJobdesk?.nama_perusahaan}</span>
							</li>
						)}
						<li className="flex justify-between">
						...
						<li className="flex justify-between">
							...
							<span className="text-gray-800">{detailJobdesk?.jabatan_pegawai}</span>
						</li>
						<li className="flex justify-between">
							<span className="font-medium">Total Jobdesk:</span>
							<span className="text-gray-800">{detailJobdesk?.total_jobdesk}</span>
						</li>
						<li>
							...
							<ul className="mt-2 space-y-2">
								{detailJobdesk?.jobdesk_details?.length > 0 ? (
									detailJobdesk?.jobdesk_details?.map((jobdesk, index) => (
										<li key={index} className="border border-gray-300 p-4 rounded-md bg-gray-50">
                        ...
                        <span className="text-gray-800">
                          {jobdesk?.priority_display}
											...
											<div className="mt-1">
												...
												<span className="text-gray-800">{jobdesk?.recurrence_display}</span>
											</div>
											{/* <div className="mt-1">
                        ...
                        <span className="text-gray-800">
                          {jobdesk?.is_completed ? "Selesai" : "Belum Selesai"}
											...
											<div className="mt-1">
												...
												<span className="text-gray-800">{jobdesk?.created_by_firstname}</span>
											</div>
											{/* <div className="mt-1">
